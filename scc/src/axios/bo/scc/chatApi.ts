/*
import axios from '@api/cmm/api';
import type {Chat, ChatData, Mgr, ChatFormData} from '@/types';
import { getMgrDetail } from '@api/mgr/mgrApi';
import { getLoginMgr } from '@api/cmm/loginApi';
...
export const updateChatFormText = async (chatSeq: ChatFormData['chatSeq'], text: ChatFormData['text']) => {
    if (!chatSeq || !text) return;
    const newDate = salmon.date.newDate().format('YYYY/MM/DD HH:mm:ss');
    const loginInfo = await getLoginMgr();
    try {
...
export const updateChatFormMemo = async (chatSeq: ChatFormData['chatSeq'], text: ChatFormData['text']) => {
    if (!chatSeq || !text) return;
    const newDate = salmon.date.newDate().format('YYYY/MM/DD HH:mm:ss');
    const loginInfo = await getLoginMgr();
    try {
